...
export async function GET(
  ...
  try {
    ...
    // Mark-read-on-open: bump last_user_read_at to NOW(). Returns the ticket
    // row only if it actually belongs to the requesting user, so the same
    // statement enforces ownership without a separate SELECT.
    ...
    const [messagesResult, attachmentsByMessage, reactionsByMessage] = await Promise.all([
