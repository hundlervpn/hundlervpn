...
-- reply_to_id lets a message quote an earlier one in the same ticket
-- (messenger-style swipe-to-reply / reply button). ON DELETE SET NULL so
-- deleting a quoted message keeps the reply, just drops the dangling quote.
-- Reactions: one emoji per side (user/admin) per message — re-reacting with a
-- new emoji replaces it, re-tapping the same emoji removes it (handled in API).
-- Idempotent.
ALTER TABLE support_ticket_messages ADD COLUMN IF NOT EXISTS reply_to_id BIGINT REFERENCES support_ticket_messages(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_support_ticket_messages_reply_to ON support_ticket_messages(reply_to_id);
