 ...
 *
 * **Короткий кэш 60с** через `Cache-Control` чтобы новый релиз был виден
 * клиентам в течение ~минуты. Rate-limit GitHub API не упираемся за счёт
 * PAT (`GITHUB_RELEASE_TOKEN`).
 *
...
export async function GET() {
  ...
  return NextResponse.json(
    ...
    {
      headers: {
        // Короткий кэш: edge 60с + browser 30с. Новый релиз станет виден
        // клиентам в течение ~минуты после публикации. Клиент дополнительно
        // ре-чекает раз в 15 мин и при фокусе окна.
