  ...
  try {
    ...
    if (update.message && update.message.successful_payment) {
      ...
      try {
        ...
        // Insert the payments row BEFORE applyReferralReward so the journal
        // in referral_bonus_transactions can FK to an already-existing
        // payment.id. Telegram's invoice flow doesn't retry successful_payment
        // webhooks so dedup via telegram_payment_charge_id is belt-and-braces.
        ...
        const paymentRowId = insertedPayment.rows[0]?.id ?? null;
